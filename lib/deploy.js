...
// Agent and editor tooling that must never reach a live server, excluded for every resource
// whether or not its own `.distignore` lists it. Each repo kept its own list by hand, so a new
// tooling directory shipped from all of them at once: a deploy put a whole `.beads` workspace on
// a public host, browsable, because that repo's list predated beads. Defaults come first, so a
// resource that genuinely needs one of these can still re-include it with a `!` line
...
const deploy = (projectName='default', options) => {
	const buildIgnoreParams = (distignore) => {
		return [...ALWAYS_IGNORED, ...(distignore || [])].map(item => {
		...
		['themes', 'plugins'].forEach(resourceType => {
			...
			for (let resource of resources) {
				...
				const distignorePath = path.join(resource, '.distignore'),
					// called either way: a resource with no `.distignore` still gets the defaults
					ignore = buildIgnoreParams(sh.test('-f', distignorePath) ? sh.cat(distignorePath).split('\n') : []);
