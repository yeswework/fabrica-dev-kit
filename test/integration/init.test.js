...
test('a setup.yml is written from the packaged template', () => {
	const dir = makeTmpDir('fdk-init-'),
		res = init(['mysite', '--title', 'My Site'], dir);
...
test('with no slug the folder name is used', () => {
...
test('an existing setup.yml is never overwritten', () => {
	const dir = makeTmpDir('fdk-init-');
	fs.writeFileSync(path.join(dir, 'setup.yml'), 'slug: original\n');
	const res = init(['mysite'], dir);
...
test('--create-dir makes the folder and refuses to reuse one', () => {
	const dir = makeTmpDir('fdk-init-');
	assert.equal(init(['mysite', '--create-dir'], dir).status, 0);
	assert.ok(fs.existsSync(path.join(dir, 'mysite', 'setup.yml')));

	const again = init(['mysite', '--create-dir'], dir);
	assert.equal(again.status, 1);
	assert.match(again.stderr, /already a file or folder called 'mysite'/);
});

test('--create-dir with no slug is refused', () => {
	const res = init(['--create-dir'], makeTmpDir('fdk-init-'));
	assert.equal(res.status, 1);
